...
def run_full_pipeline(
    ...
    *,
    ...
    selected_domains = [
        domain for domain, _ in domain_counts.most_common(max_domains) if domain != "general"
    ]
    if not selected_domains:
        selected_domains = [domain_counts.most_common(1)[0][0]]

    persona_counts = Counter(example.persona for example in train_examples)
    selected_persona = persona_counts.most_common(1)[0][0]
    ...
    persona_training_result = train_persona_adapter(
        persona_label=selected_persona,
        config=config,
        max_examples=max_examples_per_adapter or config.train_max_examples,
    )
    ...
    summary = {
        ...
        "trained_persona": selected_persona,
        "domain_training_results": domain_training_results,
        "persona_training_result": persona_training_result,
